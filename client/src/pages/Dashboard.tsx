...
import type { InsightsPayload, Order, OrderStatus } from "../types/order";
...
export function Dashboard() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const smart = await api<InsightsPayload>("/api/analytics/insights");
      setOrders(data);
      setInsights(smart);
    } catch (e) {
        ...
        <section className="panel mt-2">
          <div className="panel-head">
            ...
            <p className="muted small">
              Auto-generated heuristics to highlight operational risk and priority.
            </p>
          </div>
          <div className="kpi-grid">
            {insights.cards.map((c) => (
              <div key={c.key} className="kpi">
                ...
                <div className="muted small mt-1">{c.detail}</div>
              </div>
            ))}
          </div>
        </section>
